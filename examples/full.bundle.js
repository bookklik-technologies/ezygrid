(() => {
  ...
  function parseRange(range) {
    ...
    if (bang >= 0) {
      sheet2 = range.slice(0, bang).replace(/^'(.*)'$/, (_, inner) => inner.replace(/''/g, "'"));
  ...
  function tokenize(input) {
    ...
    while (i < n) {
      ...
      if (/[A-Za-z_$']/.test(ch)) {
        ...
        tokens.push({ type: isRef ? "cellref" : "ident", text, sheet: sheet2 });
        i = j;
        continue;
      }
  ...
  var Parser = class _Parser {
    ...
    parsePrimary() {
      ...
      if (t.type === "cellref") {
        ...
        return { kind: "ref", sheet: sheet2, row: addr.row, column: addr.column };
      }
      if (t.type === "ident") {
  ...
  function key(sheet2, row, column, currentSheet) {
    return `${sheet2 ?? currentSheet}!${row},${column}`;
  }
  function splitKey(k) {
    const bang = k.lastIndexOf("!");
    const sheet2 = bang >= 0 ? k.slice(0, bang) : "";
    const [row, column] = k.slice(bang + 1).split(",");
    return [sheet2, Number(row), Number(column)];
  }
  var DependencyGraph = class _DependencyGraph {
    ...
    createContext(sheet2, getRaw) {
      this.currentSheet = sheet2;
      return this.buildContext(getRaw, /* @__PURE__ */ new Set());
    }
    /** Register or update a formula at the given cell. */
    setFormula(sheet2, row, column, formula, ast) {
      const k = key(sheet2, row, column, sheet2);
      const refs = collectRefs(ast).map((r) => ({
        sheet: r.sheet ?? sheet2,
        row: r.row,
        column: r.column
      }));
      const refKeys = refs.map((r) => key(r.sheet, r.row, r.column, sheet2));
    ...
    removeFormula(sheet2, row, column) {
      const k = key(sheet2, row, column, sheet2);
    ...
    recalculate(sheet2, row, column, getRaw) {
      this.currentSheet = sheet2;
      const k = key(sheet2, row, column, sheet2);
      return this.evaluateWithDependents(k, getRaw, /* @__PURE__ */ new Set());
    }
    buildContext(getRaw, visited, sheet2) {
      const ownerSheet = sheet2 ?? this.currentSheet;
      return {
    ...
    cachedValue(sheet2, row, column) {
      return this.formulas.get(key(sheet2, row, column, sheet2))?.value ?? null;
    }
    get formulaCount() {
      let count = 0;
...
`;
  ...
  var Worksheet = class {
    ...
    setStyle(range, style2) {
      const rect = parseRange(range);
      for (let r = rect.top; r <= rect.bottom; r++) {
        for (let c = rect.left; c <= rect.right; c++) {
          const key2 = `${r},${c}`;
          const current = this.styles.get(key2) ?? {};
          this.styles.set(key2, { ...current, ...style2 });
        }
      }
    }
    clearStyle(range) {
    ...
    readRawValue(sheetName, row, column) {
      const sheet2 = sheetName === this.name ? this : this.workbook.getWorksheet(sheetName);
      if (!sheet2) return new FormulaError("#REF!");
      return sheet2.resolveEvaluatedValue(row, column);
    }
    /**
  ...
  var Workbook = class {
    ...
    constructor(options = {}) {
      ...
      this.formulaGraph.setTableResolver((table, column, _item) => {
        ...
        const tableDef = sheet2.tables.get(table);
        if (!tableDef) return new FormulaError("#NAME?");
        const rect = tableDef.range;
        const top = tableDef.headerRow ? rect.top + 1 : rect.top;
        if (column === void 0) {
          return this.matrixFromRange(sheet2, rect.left, rect.right, top, rect.bottom);
        }
        const colDef = tableDef.columns.find((c) => c.name === column);
        if (!colDef) return new FormulaError("#REF!");
        return this.matrixFromRange(sheet2, colDef.index, colDef.index, top, rect.bottom);
      });
    }
    /** Build a matrix value from a worksheet rectangle. */
    matrixFromRange(sheet2, left, right, top, bottom) {
      const values = [];
      for (let r = top; r <= bottom; r++) {
        const row = [];
        for (let c = left; c <= right; c++) {
          row.push(sheet2.readRawValue(this.activeWorksheet.name, r, c));
        }
        values.push(row);
      }
      return { kind: "matrix", rows: values.length, columns: values[0]?.length ?? 0, values };
    ...
    resolveNameValue(name) {
      ...
      if (definition.type === "value") {
        ...
        if (typeof value === "string" && value.startsWith("=")) {
          ...
          const sheet3 = this.activeWorksheet;
          const ast = Parser.parse(value.slice(1));
          const ctx = graph.createContext(sheet3.name, (s, r, c) => {
            if (r < 0 || c < 0) return null;
            return sheet3.readRawValue(s ?? sheet3.name, r, c);
          });
      ...
      const sheet2 = rect.sheet ? this.getWorksheet(rect.sheet) : this.activeWorksheet;
      if (!sheet2) return new FormulaError("#REF!");
      const values = [];
      for (let r = rect.top; r <= rect.bottom; r++) {
        const row = [];
        for (let c = rect.left; c <= rect.right; c++) {
          row.push(sheet2.readRawValue(rect.sheet ?? this.activeWorksheet.name, r, c));
        }
        values.push(row);
      }
      return { kind: "matrix", rows: values.length, columns: values[0]?.length ?? 0, values };
    ...
    addWorksheet(config = {}) {
      const sheet2 = new Worksheet(this, config);
      this.worksheets.push(sheet2);
      return sheet2;
    }
    removeWorksheet(id) {
    ...
    applyCellSet(payload, worksheetId) {
      const sheet2 = worksheetId !== void 0 ? this.worksheets.find((w) => w.id === worksheetId) : this.activeWorksheet;
      if (!sheet2) return;
      const existing = sheet2.cells.getCell(payload.row, payload.column);
      if (existing?.formula !== void 0) {
        this.formulaGraph.removeFormula(sheet2.name, payload.row, payload.column);
      }
      if (payload.formula !== void 0) {
        const ast = Parser.parse(payload.formula.slice(1));
        sheet2.cells.setCell(payload.row, payload.column, { formula: payload.formula });
    ...
    refreshFormulaGraph() {
      const graph = this.formulaGraph;
      graph.clear();
      for (const sheet2 of this.worksheets) {
        sheet2.cells.forEach((row, column, record) => {
          if (record.formula !== void 0) {
            graph.setFormula(sheet2.name, row, column, record.formula, Parser.parse(record.formula.slice(1)));
          }
        });
      }
    }
    ...
    transformFormulasForStructuralChange(sheetName, shift) {
      for (const sheet2 of this.worksheets) {
        ...
        for (const u of updates) {
          const record = sheet2.cells.getCell(u.row, u.column);
    ...
    undo() {
      ...
      if (!operation) return;
      const sheet2 = this.worksheets.find((w) => w.id === operation.worksheetId);
      if (!sheet2) return;
      const payload = operation.payload;
      this.applyCellSet(
        {
    ...
    toJSON() {
      return {
        ...
        version: 1,
        id: this.id,
        worksheets: this.worksheets.map((sheet2) => {
          const rows = [];
          const used = sheet2.cells.usedRange;
          if (used) {
            for (let r = 0; r <= used.bottom; r++) {
              const row = [];
              for (let c = 0; c <= used.right; c++) {
                const record = sheet2.cells.getCell(r, c);
                if (!record) {
                  row.push(null);
                  continue;
                }
          ...
          return {
            id: sheet2.id,
            name: sheet2.name,
            dimensions: { rows: sheet2.rowCount, columns: sheet2.columnCount },
            data: rows
          };
        })
      };
  ...
  var EditService = class {
    ...
    session_ = null;
    beginEdit(sheet2, row, column, initial) {
      const raw = sheet2.cells.getCell(row, column);
    ...
    commit(sheet2, text) {
      ...
      if (value !== "") {
        ...
        sheet2.setValue(row, column, parsed);
      } else {
        sheet2.setValue(row, column, null);
      }
      this.end();
      return true;
    }
...
<text x="${padding.left - 4}" y="${padding.top + plotH + 4}" font-size="9" fill="currentColor" fill-opacity="0.7">0</text>`;
  ...
  var GridRenderer = class {
    ...
    scrollCol = 0;
    destroyed = false;
    constructor(container, workbook, options = {}) {
  ...
  function buildThemeCss(overrides = {}) {
    ...
    const lines = Object.entries(VARIABLE_MAP).map(([key2, variable]) => `  ${variable}: ${tokens[key2]};`).join("\n");
    return `.ezygrid {
${lines}
}`;
  }

  // examples/src/full.ts
  var style = document.createElement("style");
  style.textContent = buildThemeCss({ selection: "#7c3aed", selectionSoft: "rgba(124,58,237,0.10)" });
  document.head.appendChild(style);
  var wb = createGrid(null, {
    toolbar: true,
    formulaBar: true,
    worksheets: [
      {
  ...
  var sheet = wb.activeWorksheet;
  sheet.setStyle("A1:E1", { bold: true, background: "#f4f4f5" });
  sheet.setNumberFormat("D2:D5", "#,##0.00");
  sheet.setStyle("E2:E5", { align: "center" });
  sheet.addValidation({ range: "C2:C50", type: "number", action: "reject", min: 0, max: 1e3, message: "Qty must be 0..1000" });
  ...
  document.querySelector("#status").textContent = "Try: edit Qty (validated), use the fill handle, right-click for context menu, watch the chart.";
})();
